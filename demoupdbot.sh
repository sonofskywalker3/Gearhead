...
inotifywait --monitor --format %f -e moved_to -e create ~/NSW/"0000 - (NX-eShop)"/"0000 - (DEMO Updates)" | while read FILE; 
do
  PAYLOAD="{\"content\":\"***New NSP Demo Update upload detected:*** \`\`\`$FILE\`\`\`\"}"
